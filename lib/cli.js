...
var optimist = require('optimist')
  ...
  .check(function(argv){
    ...
    if (typeof argv.w === 'string') { return true; }
    if (argv.w === true && argv._.length > 0) { return true; }
...
exports = module.exports = function(args) {
  ...
  if (argv.w) {
    ...
    watch(watchDir, function(file){
      ...
      if (isSassFile(file)) {
        options.inFile = file;
        options.outFile = path.join(cwd, path.basename(file, '.scss') + suffix);

        throttledRender = throttle(render, options, emitter);
        throttledRender();
      }
    });
